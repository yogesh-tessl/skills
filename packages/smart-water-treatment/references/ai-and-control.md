...
10. [閉迴路系統的 AI 預測實戰教訓](#閉迴路系統的-ai-預測實戰教訓)
...
下表為主要時序基礎模型的特性比較（截至 2025 年中）。選擇依據為：部署環境（雲端或邊緣）、數據量多寡、以及是否需要可解釋性。粗體標示為水處理場域已有驗證證據的模型。此領域發展迅速，使用前請確認是否有更新的模型或基準測試。
...
在閉迴路控制系統（即有 PID 或其他自動控制器運作的系統）中，AI 預測面臨根本性困難：

- **因果分離困難**：控制器產生的反饋信號與製程擾動信號高度相關，導致因果關係難以透過數據觀測分離
- **PID 信號吸收**：在多槽串聯系統中，中間槽的 PID 控制器會吸收來自上游的擾動信號。PID 的設計目標是消除偏差，等同把上游傳來的可預測信號修正掉，只剩不可預測的殘差傳往下游。結果是跨槽水質相關性通常遠低於水力連通性所暗示的水準
- **持續性基線天然強勢**：PID 控制下的系統具有高自相關性（autocorrelation），「下一步輸出 ≈ 當前輸出」是極強的預測基線（persistence baseline）

> 💡 **小知識：為什麼「不變」常常是最準的預測？**
> 在調校良好的 PID 系統中，控制器會積極修正偏差，讓系統保持在設定值附近。這意味著大部分時間「下一分鐘 ≈ 這一分鐘」——AI 要贏過這個看似愚蠢但實際很強的基線，比想像中困難得多。PID 越粗糙的系統，AI 能發揮的空間越大。

**來源**：
- [Tandfonline 2020](https://www.tandfonline.com/doi/full/10.1080/23311916.2020.1796895) — 閉迴路系統辨識的實務分析
- [ScienceDirect](https://www.sciencedirect.com/science/article/abs/pii/S0098135417304179) — 控制性能與持續激勵之間的權衡

### 預測型 vs 處方型模型

閉迴路系統中 AI 模型有兩種根本不同的定位：

| 定位 | 目標 | 特性 |
|------|------|------|
| **預測型** | 預測「控制器會輸出多少」 | 本質是複製控制器行為，在調校良好的系統中改善空間極有限 |
| **處方型** | 建議「控制器應該輸出多少」 | 用歷史上良好操作時段的資料作為參考基準，AI 輸出建議值 |

判斷依據：若 AI 對 PID 輸出的預測幾乎無法超越持續性基線，應轉向處方型定位。

處方型模型的要件：
- 必須明確定義「良好操作」的篩選標準（如低偏離事件、穩態運轉、出水達標等）
- 模型輸出的是「建議值」而非「預測值」，操作員或自動系統可選擇是否採用
- 需要搭配信心指數，讓系統知道何時該信任 AI 建議、何時該回退到 PID

### 事件起始（Onset）vs 延續（Continuation）檢測

事件預測必須發生在事件起始之前才有實際價值。事件開始後的偵測只是「延續確認」而非「預警」。

**水處理系統中常見的標籤陷阱**：
- 偏離事件的標籤若包含「已偏離的延續時段」，模型會學到「偵測當下偏離」而非「預警即將偏離」
- 改為起始標籤（onset label）後，模型表觀效能通常會大幅下降——這反映的是真實預警難度，而非模型退步

正確做法：
1. 起始標籤只標記事件發生前的時間窗口，排除已偏離時段
2. 若起始標籤下模型幾乎無效，說明現有特徵不含事件前兆信號，需引入額外資訊（如上游進水成分、製程排程）
3. 可考慮層級式方法：先偵測前兆狀態（如波動性升高），再偵測實際事件

**來源**：
- [ICML 2023 — Yeche et al.](https://proceedings.mlr.press/v202/yeche23a/yeche23a.pdf) — "Temporal Label Smoothing for Early Event Prediction"
- [Springer ML 2022](https://link.springer.com/article/10.1007/s10994-022-06300-x) — 層級式關鍵健康事件預測方法

### 模型複雜度選擇

線性模型在水質預測中經常表現不亞於複雜模型。

複雜度選擇的實務原則：
- 在 PID 控制的閉迴路系統中，可預測的變異已被 PID 吸收，剩餘信號通常近似線性
- 若非線性模型（如梯度提升樹）相較線性模型（如 Ridge 迴歸）改善幅度不大，應優先選擇簡單模型（可解釋性更高、過擬合風險更低）

**關鍵警示**：若複雜模型大幅勝出，優先檢查是否存在目標洩漏（data leakage），而非慶祝。常見洩漏源包括：
- 特徵中包含目標變數的衍生值（如 PID 輸出的差分值用來預測 PID 輸出）
- 滾動特徵跨越訓練/測試邊界
- 工況聚類使用了測試集資料

**來源**：
- [MDPI Smart Cities 2023](https://www.mdpi.com/2624-6511/6/5/126) — 多元線性迴歸在水質預測中解釋 99% 變異量

### 水處理時序特徵工程的通用原則

從多個閉迴路水處理系統歸納的特徵有效性排序：

> 下表依實務有效性排序，波動性衍生特徵通常是最強的預測因子。判讀重點：在閉迴路系統中，經 PID 控制的上游信號因已被修正，直接使用反而可能引入噪音。

| 優先級 | 特徵類型 | 說明 |
|--------|---------|------|
| **高** | 波動性衍生特徵（如移動標準差、變化率） | 捕捉系統不穩定的前兆，通常比原始感測器值更有預測力 |
| **中** | 歷史滯後值（lag features） | 捕捉系統慣性，但邊際貢獻遞減 |
| **低/須謹慎** | 經 PID 控制的上游信號 | PID 已吸收大部分可預測信號；直接使用可能引入噪音而非資訊 |
| **低** | 時間特徵（如小時、班別） | 在連續製程中價值有限；在批次製程中可能更有用 |

通用原則：
- 感測器衍生特徵（動態指標）往往勝過原始值
- 上游信號若經過 PID 控制，應監控其 PID 輸出變化率（而非水質值本身）
- 特徵數超過 20-30 個後通常出現資訊冗餘，精簡特徵集反而更穩健

### 漸進式部署策略

AI 加藥控制系統的漸進式部署框架（通用於化學加藥場景）：

> 下表列出從模仿到半自動的四階段部署路線。每個階段都有明確的進入條件與退出指標，確保安全漸進。

| 階段 | AI 角色 | 關鍵驗證指標 |
|------|---------|-------------|
| Layer 0：模仿 | 用線性迴歸擬合既有 PID 行為 | AI 輸出與 PID 一致性 >95% |
| Layer 1：影子 | 並行運算但不控制；加入 PID 看不到的信號（化學劑流量、上游狀態） | AI 建議 vs PID 實際的偏差分佈是否合理 |
| Layer 2：建議 | SCADA 顯示 AI 建議值供操作員參考 | 操作員採納率、殘差趨勢 |
| Layer 3：半自動 | 操作員可選擇讓 AI 接管控制 | 整體藥劑消耗、出水達標率 |

安全機制設計：
- **信心指數**：基於 AI 與 PID 輸出差異的連續分數，超過閾值時自動回退到 PID
- **降級機制**：AI 層故障時自動回退到 PID（安全聯鎖獨立於 AI 層）
- **每階段設定明確的進入/退出指標**（Go/No-Go）：準確度、殘差趨勢、操作員接受度、出水合規率

**來源**：
- [ScienceDirect 2024](https://www.sciencedirect.com/science/article/abs/pii/S2214714424006639) — 某研究在特定條件下，軟感測器搭配機理模型可即時調整加藥，比 PID 節省 20.4% 化學藥劑（實際效益因場景而異）
...
- [semiconductor.md](semiconductor.md) — CMP/CuCMP 廢水處理（第七節）：PAC 混凝化學、銅沉澱 pH 依賴性、SCADA 資料處理
- [troubleshooting.md](troubleshooting.md) — 化學加藥系統故障排除（第八節）：PAC 酸化對沖迴路、跨槽延遲鏈診斷
