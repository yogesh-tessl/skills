...
### 來源品質分級（Source Quality Tiering）

引用數據時，依來源可信度分為四級，優先使用高等級來源，並在回覆中標示來源等級：

| 等級 | 來源類型 | 範例 | 使用原則 |
|------|---------|------|---------|
| **A — 權威標準** | 國際/產業標準組織、教科書公理 | SEMI F63、IEC 62443、ISA-101、IPCC AR6、熱力學常數表 | 直接引用，不需額外驗證 |
| **B — 同儕審查** | 學術期刊、政府技術報告 | ScienceDirect、Nature、EPA 技術文件 | 可直接引用，標註年份與 DOI |
| **C — 產業慣例** | 設備商技術指引、產業白皮書、工程手冊 | 膜商設計手冊、水務工程教科書典型值 | 引用時標註「產業慣例值」，需交叉比對 |
| **D — 待驗證** | 商業網站、百科全書、AI 模型儲備知識 | Wikipedia、商業公司技術頁面 | 僅作為線索，必須以 A/B 級來源驗證後才可採用 |

> ⚠️ 引用 D 級來源時，必須明確標示「此數值來自非權威來源，已用第一原理驗證」或「此數值待以權威來源確認」。
...
- ISA-5.5 製程顯示圖形符號：人機介面畫面上的設備符號比工程圖（P&ID）簡化，搭配 ISA-101 色彩規則表達設備狀態
- ISA-18.2 警報合理化：穩態下每位操作員每小時不超過 6 個警報；抑制干擾性警報
- EEMUA 191 警報系統成熟度：四層分級（過載 → 反應式 → 穩定 → 強健）；用量化績效指標追蹤改善
